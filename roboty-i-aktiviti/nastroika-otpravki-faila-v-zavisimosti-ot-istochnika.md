...
Также ID файла можно получить автоматически в бизнес-процессе с помощью активити «Детально об объекте Диска» (Disk.File.Get).

## **Указание ID поля, в котором находится отправляемый файл**

Чтобы узнать ID поля типа «Файл» в сущности CRM:

1. Перейдите в CRM → Настройки → Настройки форм и отчётов → Пользовательские поля.
...
* Telegram позволяет отправлять файлы до 50 МБ (ограничение API бота). Файлы большего размера отправить не получится.
* Для изображений рекомендуется использовать прямую ссылку также в случае оформления сообщения с превью (если применяется активити «Отправка оформленного сообщения»).
* При использовании короткой ссылки Битрикс24 убедитесь, что файл доступен публично (без требования авторизации).
