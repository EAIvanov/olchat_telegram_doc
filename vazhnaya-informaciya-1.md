...
{% hint style="warning" %}
В Олчат: Telegram для коннектора может появиться статус **«Ограничение Telegram (flood)»**.

Это означает, что на вашем Telegram-аккаунте действует временное ограничение из-за жалоб пользователей. В таком случае обязательно проверьте информацию через бота.

В период действия такого ограничения отправка первых сообщений новым контактам через Олчат будет заблокирована. Вы сможете отвечать только тем, кто написал вам первым.

После появления статуса «Ограничение Telegram (flood)» сразу проверьте бота, дождитесь снятия ограничения и только потом возобновляйте массовую рассылку или первые контакты.
{% endhint %}
...
У пользователя действительно нет Telegram, номер не зарегистрирован в приложении.

Во всех перечисленных случаях вы увидите уведомление при попытке написать из Битрикс24:&#x20;
