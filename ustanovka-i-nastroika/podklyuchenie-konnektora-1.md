...
# Copy of Подключение открытой линии и коннектора

После установки перейдите в приложение «Олчат: Telegram» в левом меню портала:
...
{% hint style="info" %}
При первичном подключении вашего номера к коннектору запускается синхронизация данных. Для её корректного завершения необходимо подождать около 10 минут.
{% endhint %}
...
Если в настройках конфиденциальности вашего аккаунта Telegram подключена двухэтапная аутентификация (Облачный пароль), после ввода кода подтверждения на предыдущем шаге вам потребуется ввести ваш Облачный пароль в специальное поле ввода 2FA-пароля (пароля двухфакторной аутентификации):

<figure><img src="../.gitbook/assets/image (996).png" alt=""><figcaption></figcaption></figure>

После подключения вы увидите статус «Аккаунт авторизован», ID (идентификатор) и Username имя) подключенного пользователя, а также номер телефона, для которого подключена интеграция.

<figure><img src="../.gitbook/assets/image (987).png" alt=""><figcaption></figcaption></figure>

### Авторизация по ссылке

Также есть возможность авторизации по специальной ссылке. Если подключиться нужно другому человеку (например, коллеге или администратору), нажмите кнопку **Скопировать ссылку на авторизацию**. Полученную ссылку можно отправить этому человеку. По ней он сможет пройти авторизацию в своем Telegram без необходимости передавать номер телефона или коды подтверждения.

<figure><img src="../.gitbook/assets/388efd6a-2a6f-4bec-8f4c-561493583ad9.png" alt=""><figcaption></figcaption></figure>

Эта функция особенно удобна, когда настройку выполняет не владелец номера.
