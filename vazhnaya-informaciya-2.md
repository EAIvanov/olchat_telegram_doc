...
# Copy of Блокировка номера

Наличие номера телефона в CRM не всегда означает, что вы сможете отправить сообщение через интеграцию. Telegram имеет несколько уровней защиты от спама и жёсткие настройки приватности. Ниже описаны основные причины, по которым сообщение может не отправляться.
...
### Настройки конфиденциальности Premium-аккаунта

Если у пользователя, которому вы хотите написать, включён **Telegram Premium** и в настройках конфиденциальности установлено: **«Кто может отправлять мне сообщения? → Контакты и Premium-подписчики»**, а ваш аккаунт не является Premium, то:

* В интерфейсе Олчат напротив контакта будет отображаться зелёная точка (система считает, что писать можно).
* При попытке отправить сообщение появится стандартная ошибка: «Не удалось отправить сообщение. На данный момент отправка сообщения недоступна».

<figure><img src=".gitbook/assets/image (1063).png" alt=""><figcaption></figcaption></figure>

В таком случае рекомендуется проверить возможность написать с телефона (через обычное приложение Telegram). Скорее всего, там вы увидите сообщение:

<figure><img src=".gitbook/assets/image (1064).png" alt=""><figcaption></figcaption></figure>

Это ограничение Telegram, и обойти его через интеграцию нельзя.

## Лимиты

Вы узнали, что у пользователя номер виден всем, но написать все равно не получается? Помимо настроек конфиденциальности, существует также ограничение Telegram на отправку сообщений новым контактам. Telegram специально встроил механизмы защиты от спама, которые ограничивают возможность писать первыми незнакомым людям.
