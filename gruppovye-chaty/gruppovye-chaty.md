...
{% hint style="info" %}
Команда OLChat никак не может повлиять на смену типа группы с обычной на супергруппу. Данные изменения происходят исключительно на стороне Telegram и могут произойти при выполнении даже одного пункта из списка выше. Для корректной работы рекомендуем сразу переводить тип группы в супергруппу.
{% endhint %}

При смене типа группы происходит и смена ID группы. В настройках групп OLChat Telegram ID активных супергрупп отображается в формате "100111111", т.е. всегда начинается с "100".

В случае, когда произошла смена типа группы, в приложении OLChat Telegram будет дублирование групп. Написать в старую группу больше не получится - кнопки "Открыть чат" не будет. Используйте для работы ту группу, которая имеет тип "Супергруппа" и её ID начинается с "100...".

Тип группы можно также отличить по значку в настройках приложения OLChat Telegram:
